...
**2026-06-02** — Full dream-cycle pass. 69 wiki pages analyzed (48 concepts, 15 topics, 19 people + topics). Inbound-edge counts computed from `related:` fields and in-text `[[wikilinks]]`. Body depth estimated in words. Staleness is uniformly low (vault created 2026-05-18 to 2026-05-20; all pages 12–15 days old), so staleness does not differentiate. No true orphans found.
...
| `large-language-models` | 6 | ~250 | Foundational anchor for the AI cluster; referenced by nearly every AI-related page. Body is a brief properties list; lacks history of the transformer, capability trajectory, societal framing, and failure modes. |
...
| `adaptive-vs-technical-challenges` | 3 | unknown | Referenced by `adaptive-leadership` and `adaptive-leadership-and-psychological-safety`; core Heifetz diagnostic tool. Not yet read in this pass — flag for depth check. |
...
All pages share the same age band (2026-05-18 to 2026-05-20). Staleness does not differentiate within this vault at its current age. Re-run this section after the next enrichment pass to establish a baseline.

**Exception — uningested raw articles (aging gap):** Two PDF files in `raw/articles/` have never been ingested, meaning the pages that would most benefit from them have been aging without the primary source:
- `raw/articles/charlie munger - mental models Part 1.pdf`
- `raw/articles/charlie munger mental Models Part 2.pdf`

These are likely the transcript(s) of Munger's 1994 USC "Elementary Worldly Wisdom" talk or related primary writings — the original source for the latticework framework. The pages `mental-models`, `latticework-pedagogy`, `munger-disposition-formation`, and `mental-models-and-multidisciplinary-thinking` have all been built from the *Poor Charlie's Almanack* epub and secondary references; these PDFs may contain primary material not yet captured.

---

## Near-duplicate flags (Stage 2)
_Near-duplicate pages proposed for consolidation — never delete, owner approves any merge or trim._
...
**Proposed for each:** Compress the inline answer to a 1–2 sentence summary + wikilink to the concept page. The concept page becomes canonical; the topic page retains the question header and a brief pointer. **Owner must approve each compression individually** — the inline text may contain nuances not fully replicated in the concept page.
